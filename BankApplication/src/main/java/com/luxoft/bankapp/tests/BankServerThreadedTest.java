...
public class BankServerThreadedTest {
	/*
	 * @Test public void testBankServer(){
	 * 
	 * try { BankCommander.currentBank= BankApplication.initialize(); } catch
	 * (ClientExistsException e1) { // TODO Auto-generated catch block
	 * e1.printStackTrace(); }
	 * 
	 * BankServiceImpl bankService = new BankServiceImpl();
	 * BankCommander.currentClient =
	 * bankService.findClientByHisName(BankCommander.currentBank,"Tom");
	 * 
	 * double ammount =
	 * BankCommander.currentClient.getActiveAccount().getBalance();
	 * 
	 * 
	 * BankServerThreaded bankServerThreaded = new BankServerThreaded(9090);
	 * Thread server = new Thread(bankServerThreaded); server.start();
	 * 
	 * List<Thread> mocksClients = new ArrayList<>();
	 * 
	 * for (int i = 0; i < 1000; i++) { mocksClients.add(new Thread(new
	 * BankClientMock())); }
	 * 
	 * for (Thread thread : mocksClients) { thread.start(); }
	 * 
	 * for (Thread thread : mocksClients) { try { thread.join(); } catch
	 ...
	 * double ammount2 =
	 * BankCommander.currentClient.getActiveAccount().getBalance();
	 * System.out.println(ammount2); Assert.assertEquals(ammount-1000, ammount2,
	 * 0); }
	 */
	@Test
